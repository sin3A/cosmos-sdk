...
const (
	...
	ProofOpValue = "simple:v"

	KeyTotal    = "total"
	KeyIndex    = "index"
	KeyLeafHash = "leaf_hash"
)
...
func ProofOf(proof tmmerkle.Proof) (*Proof, error) {
	var pf Proof
	for _, op := range proof.Ops {
		if op.Type != ProofOpValue {
			return nil, errors.New("invalid proof type")
		}
		switch string(op.Key) {
		case KeyTotal:
			pf.Total = BytesToInt64(op.Data)
		case KeyIndex:
			pf.Index = BytesToInt64(op.Data)
		case KeyLeafHash:
			pf.LeafHash = op.Data
		default:
			pf.Aunts = append(pf.Aunts, op.Data)
		}
	}
	return &pf, nil
}
...
func (sp *Proof) ValidateBasic() error {
	...
	if len(sp.LeafHash) != hasher.Size() {
		return fmt.Errorf("expected LeafHash size to be %d, got %d", hasher.Size(), len(sp.LeafHash))
	}
	...
	for i, auntHash := range sp.Aunts {
		if len(auntHash) != sm3.New().Size() {
			return fmt.Errorf("expected Aunts#%d size to be %d, got %d", i, sm3.New().Size(), len(auntHash))
		}
	}
	return nil
}

func (sp *Proof) TmProof() tmmerkle.Proof {
	var ops []tmmerkle.ProofOp

	ops = append(ops, tmmerkle.ProofOp{
		Type: ProofOpValue,
		Key:  []byte(KeyTotal),
		Data: Int64ToBytes(sp.Total),
	})

	ops = append(ops, tmmerkle.ProofOp{
		Type: ProofOpValue,
		Key:  []byte(KeyIndex),
		Data: Int64ToBytes(sp.Index),
	})

	ops = append(ops, tmmerkle.ProofOp{
		Type: ProofOpValue,
		Key:  []byte(KeyLeafHash),
		Data: sp.LeafHash,
	})

	for i, aunt := range sp.Aunts {
		ops = append(ops, tmmerkle.ProofOp{
			Type: ProofOpValue,
